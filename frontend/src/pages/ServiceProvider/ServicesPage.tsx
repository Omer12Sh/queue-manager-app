...
export default function ServicesPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm(t('services.delete') + '?')) return;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">{t('services.subtitle')}</p>
        </div>
      ...
      {showForm && (
        <div className="card border-brand-200 border-2">
          <h2 className="font-semibold text-gray-900 mb-4">{editingId ? t('services.edit') : t('services.addService')}</h2>
          <form onSubmit={handleSubmit} className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('services.descriptionLabel')}</label>
            ...
            <div className="sm:col-span-2 flex gap-3 justify-end">
              <button type="button" onClick={() => setShowForm(false)} className="btn-secondary">{t('services.cancel')}</button>
              <button type="submit" disabled={saving} className="btn-primary">{saving ? t('services.saving') : t('services.save')}</button>
            </div>
          </form>
        </div>
      )}
      ...
      {services.length === 0 ? (
        <EmptyState
          ...
          action={<button onClick={() => setShowForm(true)} className="btn-primary">{t('services.addService')}</button>}
        />
      ) : (
